...
func NewPeerService(consortium, orgDomain, peerDomain, mspID string, ports, hosts []string) []byte {
	...
	cryptoRoot := filepath.Join(utils.BlockchainRoot, "organizations", "crypto", "organizations")
		...
		"services": map[string]interface{}{
			peerDomain: map[string]interface{}{
				...
				"volumes": []string{
					...
					fmt.Sprintf("%s/organizations/crypto/organizations/peerOrganizations/%s/peers/%s/msp:/etc/hyperledger/fabric/msp", utils.BlockchainRoot, orgDomain, peerDomain),
					fmt.Sprintf("%s/organizations/crypto/organizations/peerOrganizations/%s/peers/%s/tls:/etc/hyperledger/fabric/tls", utils.BlockchainRoot, orgDomain, peerDomain),
				},
				"ports":       ports,
				"extra_hosts": hosts,
				"environment": []string{
...
func NewOrdererService(orgDomain, ordererDomain, mspID, genesisBlockPath string, ports, hosts []string) []byte {
	mspPath := filepath.Join(utils.BlockchainRoot, "organizations", "crypto", "organizations", "ordererOrganizations", orgDomain, "orderers", ordererDomain, "msp")
	tlsPath := filepath.Join(utils.BlockchainRoot, "organizations", "crypto", "organizations", "ordererOrganizations", orgDomain, "orderers", ordererDomain, "tls")
